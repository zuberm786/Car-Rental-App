  ...
  return (
    <div className="bg-white shadow-lg h-16 flex justify-between items-center md:px-[10%]">
      ...
      <div
        ...
        } gap-10 md:flex-row md:static bg-white md:w-auto text-center
       `}
      >
        {links?.map((link) => (
          <div key={link.link}>
            <Link
              className={`${
                pathname === link.link ? "text-third-color" : "text-black hover:bg-blue-500 cursor-pointer rounded-full hover:text-white  "
              }`}
              href={link.link}
            >
              <span>{link.text}</span>
